    ...
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} className={className}>
      <defs>
        ...
        <filter id={`shadow-${uid}`}>
          <feDropShadow dx="0" dy={s * 0.02} stdDeviation={s * 0.04} floodColor={colors.orange} floodOpacity="0.3" />
        </filter>
      </defs>

      {/* No background rect — transparent */}
      <ellipse cx={cx} cy={cy} rx={s * 0.3} ry={s * 0.2} fill={colors.orange} fillOpacity="0.04" />
      ...
      <line x1={cx} y1={tvY} x2={cx + antLen} y2={tvY - antLen}
        stroke={`url(#gradGold-${uid})`} strokeWidth={Math.max(1.5, s * 0.016)} strokeLinecap="round" />
      <circle cx={cx - antLen} cy={tvY - antLen} r={antDot} fill={colors.orange} filter={`url(#glow-${uid})`}>
      ...
      <polygon
        ...
        fill={colors.text} fillOpacity="0.95" filter={`url(#glow-${uid})`} />

      {/* Stand */}
      <line x1={cx - s * 0.06} y1={tvY + tvH + s * 0.03} x2={cx - s * 0.1} y2={tvY + tvH + s * 0.06}
        stroke={colors.textDim} strokeWidth={Math.max(1.5, s * 0.016)} strokeLinecap="round" />
      <line x1={cx + s * 0.06} y1={tvY + tvH + s * 0.03} x2={cx + s * 0.1} y2={tvY + tvH + s * 0.06}
        stroke={colors.textDim} strokeWidth={Math.max(1.5, s * 0.016)} strokeLinecap="round" />
      <line x1={cx - s * 0.12} y1={tvY + tvH + s * 0.06} x2={cx + s * 0.12} y2={tvY + tvH + s * 0.06}
        stroke={colors.textDim} strokeWidth={Math.max(2, s * 0.018)} strokeLinecap="round" />
      ...
      {showCP && (
        <>
          <text x={cx - s * 0.075} y={tvY + tvH + s * 0.18}
            ...
            fontSize={s * 0.11} fontWeight="800" fill={colors.orange} textAnchor="middle"
            letterSpacing={-s * 0.003}>C</text>
          <text x={cx + s * 0.065} y={tvY + tvH + s * 0.18}
            fontFamily="'Plus Jakarta Sans', sans-serif"
            fontSize={s * 0.11} fontWeight="300" fill={colors.textMuted} textAnchor="middle"
