      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-6 max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          {STAT_CARDS.map((card, i) => (
            ...
            >
              ...
              <h3 className="text-[17px] font-bold mb-3" style={{ color: colors.text }}>{card.label}</h3>
              ...
              <p className="text-[28px] font-black" style={{ color: colors.text }}>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {QUICK_BUTTONS.map((btn, qi) => (
            ...
            >
              <btn.icon size={20} style={{ color: colors.gold, filter: "drop-shadow(0 0 4px rgba(201,168,76,0.3))" }} />
              <span className="text-[11px] font-medium" style={{ color: colors.textMuted }}>{btn.label}</span>
            </button>
          ))}
        </div>
    ...
    <div className="shrink-0 px-8 py-3 flex items-center justify-between"
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2">
          <Fingerprint size={14} style={{ color: colors.gold, filter: "drop-shadow(0 0 4px rgba(201,168,76,0.4))" }} />
          <span className="text-[11px]" style={{ color: colors.textDim }}>MAC</span>
      ...
      <div className="flex items-center gap-6">
        {exp && (
          <div className="flex items-center gap-2">
            <span className="text-[11px]" style={{ color: colors.textDim }}>Expire</span>
            <span className="text-[12px] tracking-wide" style={{
              color: exp.days > 30 ? "#34C759" : exp.days > 7 ? "#FFD60A" : colors.red,
              fontFamily: "'Roboto Mono', 'SF Mono', monospace",
              fontWeight: 400,
            }}>
              {exp.date}
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-4 px-5 py-4 relative z-10">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl"
            ...
            <Crown size={22} style={{ color: colors.gold, filter: "drop-shadow(0 0 6px rgba(201,168,76,0.5))" }} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[15px] font-bold" style={{ color: colors.text }}>
              CHOUF Play <span style={{ color: colors.gold }}>Premium</span>
            </p>
            <p className="text-[11px] mt-0.5" style={{ color: colors.textMuted }}>
              Multi-playlists, EPG, Xtream, PiP, thèmes personnalisés
            </p>
            <p className="text-[12px] font-bold mt-1" style={{ color: colors.gold }}>8,99 €/an · 24,99 € à vie</p>
          </div>
      ...
      {qrOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }}
          ...
          >
            <Crown size={32} style={{ color: colors.gold, margin: "0 auto 12px", filter: "drop-shadow(0 0 10px rgba(201,168,76,0.5))" }} />
            <h3 className="text-lg font-bold mb-2" style={{ color: colors.text }}>Activer Premium</h3>
            <p className="text-[12px] mb-4" style={{ color: colors.textMuted }}>Scannez le QR code pour accéder au paiement</p>
            ...
            <p className="text-[11px] mb-3" style={{ color: colors.textDim }}>{paymentUrl}</p>
            <button onClick={() => setQrOpen(false)}
              className="rounded-xl px-6 py-2 text-[13px] font-semibold transition-colors hover:opacity-80"
              style={{ background: "rgba(201,168,76,0.15)", color: colors.gold }}>
              Fermer
            </button>
          </motion.div>
        </div>
