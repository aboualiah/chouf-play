...
export default function DemoChannels() {
  ...
  return (
    <div className="flex h-screen flex-col" style={{ background: colors.background }}>
      {/* Header */}
      <header className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: "rgba(10,10,15,0.85)", backdropFilter: "blur(12px)", borderBottom: "1px solid #1C1C24" }}>
        <button onClick={() => navigate("/", { replace: true })} className="rounded-xl p-2.5 transition-all hover:scale-105 active:scale-95 group" style={{ background: "linear-gradient(135deg, #1C1C24, #242430)", border: "1px solid #2A2A36" }}>
          <ArrowLeft size={18} className="transition-colors" style={{ color: colors.orange }} />
        </button>
        <Radio size={18} style={{ color: colors.orange }} />
        <h1 className="text-base font-bold" style={{ color: colors.text }}>Chaînes Gratuites — Démo</h1>
        <span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ background: "rgba(255,109,0,0.1)", color: colors.orange }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Channel list */}
        <div className="w-full max-w-md flex flex-col border-r overflow-hidden" style={{ background: colors.surfaceSolid, borderColor: colors.surfaceSolid2 }}>
          {/* Category filter */}
          <div className="flex gap-1.5 px-3 py-2.5 overflow-x-auto scrollbar-none shrink-0" style={{ borderBottom: "1px solid #1C1C24" }}>
            <button
              ...
              style={!activeCategory ? { background: "rgba(255,109,0,0.12)", color: colors.orange } : { background: colors.surfaceSolid2, color: colors.textDim }}
            >
              Toutes
            </button>
            {categories.map(cat => (
              <button
                ...
                className="shrink-0 rounded-lg px-3 py-1 text-[11px] font-semibold transition-colors"
                style={activeCategory === cat ? { background: "rgba(255,109,0,0.12)", color: colors.orange } : { background: colors.surfaceSolid2, color: colors.textDim }}
              >
                {cat}
              </button>
            ))}
              ...
              >
                ...
                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg overflow-hidden bg-gradient-to-br ${CATEGORY_GRADIENTS[ch.category] || "from-gray-700/30 to-gray-900/20"}`}>
                  {ch.logo ? (
                    ...
                    <span className="text-[9px] font-bold" style={{ color: colors.text }}>{getInitials(ch.name)}</span>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[12px] font-medium truncate" style={{ color: activeChannel?.id === ch.id ? "#F5F5F7" : colors.textMuted }}>{ch.name}</p>
                  <p className="text-[10px]" style={{ color: colors.textDim }}>{ch.category}</p>
                </div>
                {activeChannel?.id === ch.id ? (
                  ...
                  <Play size={12} className="opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: colors.orange }} />
                )}
              </button>
            ))}
          </div>
            ...
            {activeChannel ? (
              ...
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div className="text-center">
                  <Radio size={48} style={{ color: colors.surfaceSolid2 }} className="mx-auto mb-4" />
                  <p className="text-sm font-medium" style={{ color: colors.textDim }}>Sélectionnez une chaîne pour commencer</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
